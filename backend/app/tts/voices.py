...
ALIYUN_VOICES = [
    ...
    {"id": "longanlufeng", "name": "龙安陆风", "tags": ["沉稳", "男声"], "recommended_scene": "meditation"},
]
...
def voices_for(engine: str) -> list[dict]:
    return ALIYUN_VOICES if engine == "aliyun" else VOLC_VOICES if engine == "volc" else []


def voice_by_id(engine: str, voice_id: str) -> dict | None:
    return next((voice for voice in voices_for(engine) if voice["id"] == voice_id), None)
