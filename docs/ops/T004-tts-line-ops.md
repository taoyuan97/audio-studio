...
1. 打开 `/settings?tab=voices`，确认设置页包含“模型与环境设置”和“音色配置”两个 Tab。
2. 点击“新增音色”，分别填写阿里云模型 ID、完整音色 ID 和可选名称。Qwen-Audio 3.0 基础音色不能只填写试听文件后缀；例如 plus 模型的 `longlinshuoxi` 应填写为 `qwen-audio-3.0-tts-plus-longlinshuoxi`。表单发现疑似后缀时会提示补全，也允许按原 ID 保存以兼容声音复刻音色。保存本身不会调用供应商，也不会产生合成费用。
3. 新记录应显示“未验证”。点击“试听验证”会合成一条短句，可能产生少量费用；成功后应显示“已验证”和验证时间，失败时配置仍应保留并显示失败原因。
4. 再次普通试听应读取缓存；“重新验证”会绕过缓存并再次产生调用，必须在确认提示后执行。
5. 回到 `/tts`，当前模型匹配的自定义音色应出现在“我的音色”分组中，并可用于试听和正式生成。切换冥想/播客场景后，自定义音色选择应保持不变。
6. 自定义音色绑定其他模型时会继续保存在设置页，但不会出现在当前模型的 TTS 下拉框中；切换 `ALIYUN_TTS_MODEL_ID` 并重启后端后再核对。
7. 删除操作只删除当前 `DATA_DIR/audio.sqlite3` 中的本地配置及对应试听缓存，不会删除阿里云账号中的远端音色，也不会改写历史产物。

连接同一个后端、使用同一个 `DATA_DIR/audio.sqlite3` 的浏览器共享音色配置和验证状态；本功能不包含账号体系、云同步或跨设备数据同步。
...
backend/data/audio/previews/{engine}_{sha256(engine+model+voice_id)}.wav
```
...
选择一个尚未试听过的「引擎 + 模型 + 音色」组合：

1. 第一次点「试听」：等待真实合成并正常播放。
2. 记录后端日志中该组合的 Provider 请求次数。
3. 再次点击同一组合：应快速返回，且后端不再调用 Provider。
4. 确认 `previews/` 中新增以引擎和 SHA-256 摘要命名的 WAV；文件名不应直接包含模型或音色 ID。

自定义音色需要重新调用供应商时，优先使用设置页的“重新验证”操作并确认费用提示。系统音色如需清除缓存，应先停止后端，只处理已明确定位的单个 preview 文件；不要删除整个 `DATA_DIR`。
