...
- 自动化（pytest）：markers 解析、conversations/草稿/版本契约、LLM fake 流式事件序列、失败/取消保留旧草稿、人工草稿 revision 冲突、首次保存命名、相同内容拒绝重复版本、版本追加/恢复、现有产物幂等迁移。
- 自动化（Vitest）：ScriptView、草稿保存状态、首次保存弹窗、重复保存禁用、三项覆盖保护、版本列表/查看/恢复、useRunStream `script.draft.updated` 分发。
