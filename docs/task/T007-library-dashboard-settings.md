...
- 状态：已完成（T007-A/T007-B 与最终收尾均已实现；真实 Provider 联调统一归入 T008）
- 优先级：P0
- 类型：正式任务 7/8
...
### 1.1 历史交付拆分（2026-08-27 确认）

- **T007-A（历史阶段）**：在 T005 BGM 阻塞、T006 随之阻塞期间，先实现受限版产物库与首页。只开放 `script_meditation` 和 `voice` 的展示与单项管理；BGM、混音统一标注“后续开放”。
- **T007-B（历史阶段）**：补齐 BGM/mix 产物、完整下游闭环、清空全部、脚本历史版本恢复、设置状态与连通测试，以及完整全局运行态。
...
### 1.4 最终收尾决策（2026-09-04 确认）

- 首页不再提供“快速开始”模块；保留五个模块入口、产物统计、最近产物和运行态。
- 冥想/TTS/BGM/混音的排队中与运行中数量在侧边栏持续可见，点击对应导航项进入任务页面。
- T007 的 Provider 探测只做 mock 自动化与浏览器 UI 验收，不调用真实服务；真实服务联调统一归入 T008。
- 保留 T007-A/T007-B 的历史记录，但当前范围与验收以本节及最终验收结果为准。

## 2. 目标

实现产物库（全类型管理 + 送下游流转）、首页概览（统计/最近产物/全局运行态）、侧边栏运行态提示、设置页（配置状态、浏览器编辑、免重启热生效、连通测试）。完成后工作台各线产物形成完整管理闭环，非开发者无需编辑 `.env` 即可管理受支持的模型配置。
...
- 首页保留模块卡片入口、产物计数、最近 5 条和全类型运行态；按最终产品决策移除快速开始引导。
...
- 卡片列表：名称、类型徽章、参数摘要（脚本=当前版本号/时长/模型；voice=引擎/音色/语速/时长；bgm=自由 prompt/时长/格式；mix=轨道组合/时长/格式）、生成时间。
...
- 首页不展示快速开始模块；侧边栏按业务线展示排队中/运行中数量，点击导航项进入对应页面。
...
- 所有配置写接口携带 revision；冲突返回 409 并提示刷新，避免多个页面静默覆盖。保存或清除成功后刷新 settings status、会话 models 与 TTS/BGM defaults 等相关查询。
- 每项「测试连通」（`POST /api/settings/probe/{provider}`）：保存与探测是独立动作，loading 后显示成功延迟/失败原因；探测失败不回滚配置。探测同样执行本机 Origin 校验；FAKE_MODE 下 probe 仍明确调用真实第三方服务。
...
- 自动化（pytest）：settings status/probe/reveal 契约（掩码、来源、可回显字段、模型 ID、未配置、各 provider mock、本机 Origin、`FFMPEG_PATH`/PATH）；运行时配置优先级、原子写失败回滚、重启持久化、凭据新增/替换/保留/清除/按字段回显、`.env` 回显拒绝、超时边界、revision 冲突及全链路脱敏。
...
- 手工：四类产物入库后产物库全操作走查；删除 voice 后混音页下拉同步消失；浏览器更新服务配置后不重启验证 defaults/模型列表；确认响应、日志和页面不泄露完整凭据。真实 Provider probe 与 smoke 归入 T008。
...
- [x] 产物库支持 BGM/mix 展示、参数摘要及既定路由下游语义；BGM 与混音页面均已开放，混音空状态可直接进入 `/mixdown`。
...
- [x] 各 provider 真实探测接口与交互已实现，保存与探测互不绑定；全部设置写入、回显及探测入口均执行本机 Origin 校验，敏感信息脱敏通过验证。
- [x] 首页展示四类计数、最近产物和全局运行态，五个模块入口全部开放；快速开始已按产品决策移除，侧边栏展示各业务线排队中/运行中徽标。
...
- 后续状态：T006 混音 API/页面已完成并通过验收，首页和产物库的混音入口均已开放。
...
### 7.5 2026-09-04 T007 最终收尾验证

- 后端：`180 passed, 5 skipped`；新增全部 provider mock 探测、本机 Origin 拒绝且不触发第三方调用、超时边界、原子写失败回滚和配置快照语义测试。
- 前端：Vitest `61 passed`；ESLint 通过；TypeScript + Vite 生产构建通过。新增四类产物过滤与摘要、voice/BGM 下游、混音空状态、删除/清空确认、历史版本恢复、设置清除后的相关 Query 失效、探测/超时/revision 冲突及侧边栏运行态测试。
- 浏览器：在独立 DATA_DIR 与 FAKE_MODE 下确认首页无快速开始模块、五个模块入口开放、混音空状态按钮可进入 `/mixdown`、MiniMax Key/模型字段可见且模型默认为 `music-3.0`；通过隔离运行态测试记录确认侧边栏分别显示 TTS 排队中与 BGM 运行中徽标并可跳转；无 console error/warn。
- 安全边界：浏览器验收未点击凭据查看或任何“测试连通”，未提交真实业务生成任务，未调用真实 Provider；真实 Provider probe 与 smoke 统一归入 T008。验收临时服务和数据已清理。
