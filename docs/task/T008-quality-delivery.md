...
- [ ] E2E 主路径 A/B 绿色（含真实 FFmpeg 混音导出的文件存在性断言）。
- [ ] 排队/取消/刷新恢复/失败重试四类运行态场景 E2E 覆盖。
- [ ] 单进程部署：深链刷新、SSE 流式、音频 seek 全部经 8000 端口同源验证通过。
- [ ] dev proxy 与直连两种 SSE 验证记录留档。
- [ ] 三个 smoke 脚本输出留档（真实生成物路径与耗时）。
- [ ] README 启动说明完整（.env/ffmpeg/启动/常见错误）。
- [ ] 一期验收清单（PRD 8 节 + 原型 T10 清单映射）逐项核对通过；遗留问题列 known issues。
