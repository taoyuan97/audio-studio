...
import { artifactSummary, mixRuleText } from '../features/mixdown/rules'
...
export default function MixdownPage() {
  ...
  const payload = useMemo(() => ({
    ...
    format,
  }), [bgm, bgmGain, bgmOffset, ducking, format, voice, voiceGain])
  ...
  return <div className="mix-page">
    <div className="mix-page-head"><h2>最终混音</h2><p>组合人声与背景音乐，设置音量、偏移和人声闪避后导出成品。</p></div>
    <div className="mix-grid">
      <Card title="轨道与参数" className="mix-card">
        ...
        <div className="mix-field"><label>背景音轨 <span>可为空</span></label><Select allowClear value={bgmId} onChange={(value) => setBgmId(value ?? null)} placeholder="选择背景音乐产物" style={{ width: '100%' }}
          ...
          optionRender={(option) => renderOption(option.data.artifact as Artifact)} /></div>
        <Alert className="mix-rule" type={voiceId || bgmId ? 'info' : 'warning'} showIcon message="组合规则" description={mixRuleText(voice, bgm)} />
        <div className="mix-field"><label>人声音量 <b>{voiceGain}%</b></label><Slider aria-label="人声音量" min={0} max={100} value={voiceGain} disabled={!voiceId} onChange={setVoiceGain} /></div>
        <div className="mix-field"><label>背景音量 <b>{bgmGain}%</b></label><Slider aria-label="背景音量" min={0} max={100} value={bgmGain} disabled={!bgmId} onChange={setBgmGain} /></div>
        <div className="mix-field"><label>背景起始偏移 <b>{bgmOffset} 秒</b></label><Slider aria-label="背景起始偏移" min={0} max={60} value={bgmOffset} disabled={!bgmId || !voiceId} onChange={setBgmOffset} /></div>
      ...
      <Card title="预览与成品" className="mix-card mix-preview-card">
        ...
        : <div className="mix-preview"><DualTrackWaveform voice={voice?.audio ? { id: voice.id, label: '人声', duration: voice.audio.duration } : null} bgm={bgm?.audio ? { id: bgm.id, label: '背景', duration: bgm.audio.duration } : null} bgmOffset={bgmOffset} />{(voiceId || bgmId) && <p>{mixRuleText(voice, bgm)}</p>}{!voiceId && !bgmId && <Empty description="从左侧选择至少一条音轨" />}</div>}
      </Card>
    </div>
  </div>
}
